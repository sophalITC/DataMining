...
_DOMWorld_bindingIdentifier = { value: (name, contextId) => `${name}_${contextId}` };
const noop = () => { };
/**
 * @internal
 */
class WaitTask {
    constructor(options) {
        ...
        if (options.timeout) {
            ...
            __classPrivateFieldSet(this, _WaitTask_timeoutTimer, setTimeout(() => this.terminate(timeoutError), options.timeout), "f");
        }
        this.rerun();
    }
    terminate(error) {
    ...
    async rerun() {
        ...
        if (!error &&
            (await __classPrivateFieldGet(this, _WaitTask_domWorld, "f").evaluate((s) => !s, success).catch(() => true))) {
            if (!success)
