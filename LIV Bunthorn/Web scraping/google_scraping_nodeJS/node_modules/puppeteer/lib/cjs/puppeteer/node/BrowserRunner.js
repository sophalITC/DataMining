...
class BrowserRunner {
    ...
    start(options) {
        ...
        __classPrivateFieldSet(this, _BrowserRunner_listeners, [
            helper_js_1.helper.addEventListener(process, 'exit', this.kill.bind(this)),
        ], "f");
        if (handleSIGINT)
            __classPrivateFieldGet(this, _BrowserRunner_listeners, "f").push(helper_js_1.helper.addEventListener(process, 'SIGINT', () => {
                this.kill();
                process.exit(130);
            }));
        if (handleSIGTERM)
            __classPrivateFieldGet(this, _BrowserRunner_listeners, "f").push(helper_js_1.helper.addEventListener(process, 'SIGTERM', this.close.bind(this)));
        if (handleSIGHUP)
            __classPrivateFieldGet(this, _BrowserRunner_listeners, "f").push(helper_js_1.helper.addEventListener(process, 'SIGHUP', this.close.bind(this)));
    }
    close() {
        ...
        helper_js_1.helper.removeEventListeners(__classPrivateFieldGet(this, _BrowserRunner_listeners, "f"));
    ...
    kill() {
        ...
        if (this.proc && this.proc.pid && pidExists(this.proc.pid)) {
            ...
            try {
                ...
                throw new Error(`${PROCESS_ERROR_EXPLANATION}\nError cause: ${(0, helper_js_1.isErrorLike)(error) ? error.stack : error}`);
            }
        }
...
function pidExists(pid) {
    try {
        ...
        if ((0, helper_js_1.isErrnoException)(error)) {
