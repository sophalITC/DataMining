...
class Page extends EventEmitter_js_1.EventEmitter {
    ...
    constructor(client, target, ignoreHTTPSErrors, screenshotTaskQueue) {
        ...
        __classPrivateFieldGet(this, _Page_frameManager, "f").on(FrameManager_js_1.FrameManagerEmittedEvents.FrameAttached, (event) => this.emit("frameattached" /* PageEmittedEvents.FrameAttached */, event));
        __classPrivateFieldGet(this, _Page_frameManager, "f").on(FrameManager_js_1.FrameManagerEmittedEvents.FrameDetached, (event) => this.emit("framedetached" /* PageEmittedEvents.FrameDetached */, event));
        __classPrivateFieldGet(this, _Page_frameManager, "f").on(FrameManager_js_1.FrameManagerEmittedEvents.FrameNavigated, (event) => this.emit("framenavigated" /* PageEmittedEvents.FrameNavigated */, event));
        const networkManager = __classPrivateFieldGet(this, _Page_frameManager, "f").networkManager();
        networkManager.on(NetworkManager_js_1.NetworkManagerEmittedEvents.Request, (event) => this.emit("request" /* PageEmittedEvents.Request */, event));
        networkManager.on(NetworkManager_js_1.NetworkManagerEmittedEvents.RequestServedFromCache, (event) => this.emit("requestservedfromcache" /* PageEmittedEvents.RequestServedFromCache */, event));
        networkManager.on(NetworkManager_js_1.NetworkManagerEmittedEvents.Response, (event) => this.emit("response" /* PageEmittedEvents.Response */, event));
        networkManager.on(NetworkManager_js_1.NetworkManagerEmittedEvents.RequestFailed, (event) => this.emit("requestfailed" /* PageEmittedEvents.RequestFailed */, event));
        networkManager.on(NetworkManager_js_1.NetworkManagerEmittedEvents.RequestFinished, (event) => this.emit("requestfinished" /* PageEmittedEvents.RequestFinished */, event));
        __classPrivateFieldSet(this, _Page_fileChooserInterceptors, new Set(), "f");
        client.on('Page.domContentEventFired', () => this.emit("domcontentloaded" /* PageEmittedEvents.DOMContentLoaded */));
        client.on('Page.loadEventFired', () => this.emit("load" /* PageEmittedEvents.Load */));
        client.on('Runtime.consoleAPICalled', (event) => __classPrivateFieldGet(this, _Page_instances, "m", _Page_onConsoleAPI).call(this, event));
        client.on('Runtime.bindingCalled', (event) => __classPrivateFieldGet(this, _Page_instances, "m", _Page_onBindingCalled).call(this, event));
        client.on('Page.javascriptDialogOpening', (event) => __classPrivateFieldGet(this, _Page_instances, "m", _Page_onDialog).call(this, event));
        client.on('Runtime.exceptionThrown', (exception) => __classPrivateFieldGet(this, _Page_instances, "m", _Page_handleException).call(this, exception.exceptionDetails));
        client.on('Inspector.targetCrashed', () => __classPrivateFieldGet(this, _Page_instances, "m", _Page_onTargetCrashed).call(this));
        client.on('Performance.metrics', (event) => __classPrivateFieldGet(this, _Page_instances, "m", _Page_emitMetrics).call(this, event));
        client.on('Log.entryAdded', (event) => __classPrivateFieldGet(this, _Page_instances, "m", _Page_onLogEntryAdded).call(this, event));
        client.on('Page.fileChooserOpened', (event) => __classPrivateFieldGet(this, _Page_instances, "m", _Page_onFileChooser).call(this, event));
    ...
    async waitForRequest(urlOrPredicate, options = {}) {
        const { timeout = __classPrivateFieldGet(this, _Page_timeoutSettings, "f").timeout() } = options;
        return helper_js_1.helper.waitForEvent(__classPrivateFieldGet(this, _Page_frameManager, "f").networkManager(), NetworkManager_js_1.NetworkManagerEmittedEvents.Request, (request) => {
    ...
    async waitForResponse(urlOrPredicate, options = {}) {
        const { timeout = __classPrivateFieldGet(this, _Page_timeoutSettings, "f").timeout() } = options;
        return helper_js_1.helper.waitForEvent(__classPrivateFieldGet(this, _Page_frameManager, "f").networkManager(), NetworkManager_js_1.NetworkManagerEmittedEvents.Response, async (response) => {
            if (helper_js_1.helper.isString(urlOrPredicate))
    ...
    async waitForFrame(urlOrPredicate, options = {}) {
        ...
        let predicate;
        if (helper_js_1.helper.isString(urlOrPredicate)) {
            predicate = (frame) => Promise.resolve(urlOrPredicate === frame.url());
        }
        else {
            predicate = (frame) => {
        ...
        const eventRace = Promise.race([
            helper_js_1.helper.waitForEvent(__classPrivateFieldGet(this, _Page_frameManager, "f"), FrameManager_js_1.FrameManagerEmittedEvents.FrameAttached, predicate, timeout, __classPrivateFieldGet(this, _Page_instances, "m", _Page_sessionClosePromise).call(this)),
            helper_js_1.helper.waitForEvent(__classPrivateFieldGet(this, _Page_frameManager, "f"), FrameManager_js_1.FrameManagerEmittedEvents.FrameNavigated, predicate, timeout, __classPrivateFieldGet(this, _Page_instances, "m", _Page_sessionClosePromise).call(this)),
    ...
    async emulateTimezone(timezoneId) {
        try {
            ...
            if ((0, helper_js_1.isErrorLike)(error) && error.message.includes('Invalid timezone')) {
    ...
    async screenshot(options = {}) {
        ...
        return __classPrivateFieldGet(this, _Page_screenshotTaskQueue, "f").postTask(() => __classPrivateFieldGet(this, _Page_instances, "m", _Page_screenshotTask).call(this, screenshotType, options));
    }
    /**
    ...
    async createPDFStream(options = {}) {
        ...
        return helper_js_1.helper.getReadableFromProtocolStream(__classPrivateFieldGet(this, _Page_client, "f"), result.stream);
    }
    /**
     * @param options -
     * @returns
    ...
    async pdf(options = {}) {
        ...
        const buffer = await helper_js_1.helper.getReadableAsBuffer(readable, path);
...
_Page_closed = new WeakMap(), _Page_client = new WeakMap(), _Page_target = new WeakMap(), _Page_keyboard = new WeakMap(), _Page_mouse = new WeakMap(), _Page_timeoutSettings = new WeakMap(), _Page_touchscreen = new WeakMap(), _Page_accessibility = new WeakMap(), _Page_frameManager = new WeakMap(), _Page_emulationManager = new WeakMap(), _Page_tracing = new WeakMap(), _Page_pageBindings = new WeakMap(), _Page_coverage = new WeakMap(), _Page_javascriptEnabled = new WeakMap(), _Page_viewport = new WeakMap(), _Page_screenshotTaskQueue = new WeakMap(), _Page_workers = new WeakMap(), _Page_fileChooserInterceptors = new WeakMap(), _Page_disconnectPromise = new WeakMap(), _Page_userDragInterceptionEnabled = new WeakMap(), _Page_handlerMap = new WeakMap(), _Page_instances = new WeakSet(), _Page_initialize = async function _Page_initialize() {
    ...
    if (args)
        args.map((arg) => helper_js_1.helper.releaseObject(__classPrivateFieldGet(this, _Page_client, "f"), arg));
    if (source !== 'worker')
    ...
    const values = event.args.map((arg) => (0, JSHandle_js_1._createJSHandle)(context, arg));
    ...
    try {
        ...
        if ((0, helper_js_1.isErrorLike)(error))
...
async function _Page_setTransparentBackgroundColor() {
    ...
    if (!__classPrivateFieldGet(this, _Page_disconnectPromise, "f"))
        __classPrivateFieldSet(this, _Page_disconnectPromise, new Promise((fulfill) => __classPrivateFieldGet(this, _Page_client, "f").once(Connection_js_1.CDPSessionEmittedEvents.Disconnected, () => fulfill(new Error('Target closed')))), "f");
